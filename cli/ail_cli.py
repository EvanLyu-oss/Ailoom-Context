...
def _render_top_level_welcome() -> str:
    ...
    lines = [
        ...
        "3-command loop:",
        "1. ailoom first-run",
        "2. ailoom handoff --copy --open",
        "3. ailoom savings",
        "",
        "Share vs keep local:",
        "- Give AI: context_skeleton.mcp",
        "- Keep local: context_manifest.json and restore packages",
        "",
        "Copy/paste first:",
        "ailoom first-run",
        "",
...
def _render_context_doctor_summary(payload: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        f"- Restore safety: {restore_check.get('status', '')}",
        f"- Token savings: {metrics.get('estimated_tokens_saved', '')}",
        f"- Time: {timings.get('total', '')} ms",
        f"- Next command: {payload.get('recommended_command_text', '') or '(not available)'}",
        "",
...
def _render_context_recent_summary(payload: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        f"- Token savings: {payload.get('estimated_tokens_saved', 0)} tokens ({payload.get('estimated_savings_percent', 0)}%)",
        f"- Bundle: {payload.get('bundle_root', '') or '(not available)'}",
        f"- Next command: {payload.get('inspect_command_text', '') or payload.get('refresh_command_text', '') or '(not available)'}",
        "",
        f"Status: {payload.get('recent_status', '')}",
...
def _phase_percent(value_ms: float, *, total_ms: float) -> float:
    if total_ms <= 0:
        return 0.0
    return round((value_ms / total_ms) * 100, 2)


def _build_quick_performance_profile(
    args: argparse.Namespace,
    *,
    ...
    total_ms = float(timings_ms.get("total") or 0.0)
    raw_phases = [
        ("start_and_doctor", "Start + doctor", float(timings_ms.get("start") or 0.0)),
        ("config_recommendation", "Config recommendation", float(timings_ms.get("start_config_recommend") or 0.0)),
        ("restore_safety_check", "Restore safety check", float(timings_ms.get("start_doctor") or 0.0)),
        ("bundle_write", "Bundle write", float(timings_ms.get("bundle") or 0.0)),
    ]
    phases = [
        {
            "name": name,
            ...
            "percent_of_total": _phase_percent(duration, total_ms=total_ms),
        }
        for name, label, duration in raw_phases
    ]
    dominant_phase = max(phases, key=lambda item: float(item["duration_ms"]), default={
        "name": "quick_total",
        "label": "Quick total",
        "duration_ms": total_ms,
        "percent_of_total": 100.0 if total_ms else 0.0,
    })
    if float(dominant_phase.get("duration_ms") or 0.0) <= 0:
        dominant_phase = {
            "name": "quick_total",
            "label": "Quick total",
            "duration_ms": total_ms,
    ...
    source_summary = ((bundle_payload.get("compression") or {}).get("source_summary") or bundle_payload.get("source_summary") or {})
    default_noise = dict(source_summary.get("default_noise_protection") or {})
    skipped_dir_count = int(default_noise.get("skipped_dir_count") or source_summary.get("skipped_dir_count") or 0)
    skipped_preview = list(default_noise.get("skipped_dirs_preview") or source_summary.get("skipped_dirs") or [])[:10]
    default_noise_profile = {
        "status": str(default_noise.get("status") or "not_applicable"),
        "skipped_dir_count": skipped_dir_count,
        "skipped_dirs_preview": skipped_preview,
        "message": (
            f"default noise protection skipped {skipped_dir_count} high-noise directories"
            if skipped_dir_count
            else "default noise protection did not skip any visible directories in this run"
        ),
    }
    if default_noise_profile["status"] == "disabled":
        default_noise_profile["message"] = "default noise protection was disabled for this run"

    reuse_args = _clone_args(args, reuse_if_fresh=True, preview=False)
    next_run = {
        "recommended_strategy": "reuse_if_fresh" if not fast_path else "reuse_if_fresh_after_fast",
        "reuse_command_text": _quick_run_command_text(reuse_args),
        "fast_command_text": _quick_fast_command_text(args),
        "message": "Use --reuse-if-fresh when the project has not changed; use --fast when you want a quicker first bundle with restore safety kept on.",
    }
    return {
        "status": "available",
        "total_ms": total_ms,
        ...
        "phases": phases,
        "dominant_phase": dominant_phase,
        "default_noise_protection": default_noise_profile,
        "next_run": next_run,
    }
...
def _render_context_quick_summary(payload: dict[str, Any]) -> str:
    if payload.get("preview"):
        ...
        lines = [
            ...
            f"- Run command: {payload.get('run_command_text') or '(not available)'}",
            "",
            f"Status: {payload.get('quick_status', '')}",
            f"Restore safety: {'OK' if payload.get('restore_safe') else 'BLOCKED'}",
    ...
    lines = [
        ...
        f"- Mode: {quick_mode}",
        f"- Restore safety: {restore_safe}",
        f"- Token savings: {saved_tokens} tokens ({savings_percent}%)",
    ...
    if performance_profile.get("status") == "available":
        lines.extend([
            ...
            f"- Total measured: {performance_profile.get('total_ms', timings.get('total', 0))} ms",
            f"- Slowest phase: {dominant_phase.get('label', dominant_phase.get('name', ''))} ({dominant_phase.get('duration_ms', 0)} ms, {dominant_phase.get('percent_of_total', 0)}%)",
            f"- Phase count: {performance_profile.get('phase_count', 0)}",
            "",
            "Default noise protection:",
            f"- Status: {default_noise.get('status', '')}",
            f"- Skipped directories: {default_noise.get('skipped_dir_count', 0)}",
            f"- Message: {default_noise.get('message', '')}",
            "",
            "Next run:",
            f"- Reuse unchanged project: {next_run.get('reuse_command_text', '') or '(not available)'}",
            f"- Faster first run: {next_run.get('fast_command_text', '') or '(not available)'}",
            f"- Guidance: {next_run.get('message', '')}",
        ])
    lines.extend([
        "",
        "Recommended setup:",
...
def _build_context_quick_payload(args: argparse.Namespace) -> tuple[dict[str, Any], int]:
    ...
    if bool(getattr(args, "preview", False)):
        ...
        payload["performance_profile"] = _build_quick_performance_profile(
            args,
            start_payload=start_payload,
            bundle_payload={},
            timings_ms=timings_ms,
...
def _storage_risk_level(total_bytes: int) -> str:
...
def _parse_age_seconds(value: str | None) -> int | None:
    if not value:
        return None
    ...
    if not text:
        return None
    unit = text[-1]
    number_text = text[:-1] if unit in {"d", "h", "m", "s"} else text
    try:
        amount = float(number_text)
    except ValueError as exc:
        raise ValueError("--older-than must be a duration such as 7d, 24h, 30m, or 3600s") from exc
    multiplier = {"d": 86400, "h": 3600, "m": 60, "s": 1}.get(unit, 86400)
...
def _render_context_clean_summary(payload: dict[str, Any]) -> str:
    ...
    for target in payload.get("targets") or []:
        ...
        skipped = f", skipped: {target.get('skipped_reason')}" if target.get("skipped_reason") else ""
        lines.append(f"- {target.get('label', '')}: {exists}, {action}{skipped}, {target.get('path', '')}")
    lines.extend(
        [
            "",
            "Safety policy:",
...
def _build_context_clean_payload(args: argparse.Namespace) -> tuple[dict[str, Any], int]:
    ...
    for label, path in target_specs:
        ...
        eligible = bool(exists)
        skipped_reason = ""
        if exists and cutoff is not None and mtime > cutoff:
            eligible = False
            skipped_reason = f"newer than {older_than}"
        deleted = False
        if exists and eligible and not dry_run:
            if path.is_dir():
                shutil.rmtree(path)
            else:
                path.unlink()
...
def _render_context_storage_summary(payload: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        "",
        f"Status: {payload.get('storage_status', '')}",
        ...
        f"- Largest target: {largest.get('label', '')} ({largest.get('human_size', '')})",
        f"- Message: {storage_summary.get('message', '')}",
        "",
        "Generated artifacts:",
    ]
    for target in payload.get("targets") or []:
        lines.append(f"- {target.get('label', '')}: {target.get('human_size', '')}, {target.get('file_count', 0)} files, {target.get('path', '')}")
    lines.extend([
        "",
        "Cleanup safety:",
...
def _build_context_storage_payload(args: argparse.Namespace) -> tuple[dict[str, Any], int]:
    project_root = (_opt_path(args, "input_dir") or Path.cwd()).resolve()
    target_specs = [
        ("workspace_artifacts", project_root / ".workspace_ail"),
    ...
    risk_level = _storage_risk_level(total_bytes)
    status = "watch" if risk_level in {"watch", "large"} else "ok"
    largest_target = max(targets, key=lambda item: int(item.get("bytes") or 0)) if targets else {}
    cleanup_command = _format_cli_command(["clean", "--dry-run", "--all", "--input-dir", str(project_root)])
    storage_message = (
        "generated artifacts are very large; preview cleanup before continuing long beta runs"
        if risk_level == "large"
        else "generated artifacts are growing; consider previewing cleanup"
    ...
    payload = {
        ...
        "project_root": str(project_root),
        "total_bytes": total_bytes,
        "human_total_size": _human_size(total_bytes),
        "total_files": total_files,
        "targets": targets,
        "storage_summary": {
        ...
        "recommended_clean_command_text": cleanup_command,
    }
    payload["summary_text"] = _render_context_storage_summary(payload)
    return payload, EXIT_OK
...
def cmd_context(args: argparse.Namespace) -> int:
    ...
    if command == "doctor":
        ...
        if bool(getattr(args, "storage_check", False)):
            payload, exit_code = _build_context_storage_payload(args)
            return _emit_simple_result(args, payload, text=str(payload.get("summary_text", "")), exit_code=exit_code)
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    doctor.add_argument("--storage", dest="storage_check", action="store_true", help="Check local generated artifact size and cleanup guidance")
