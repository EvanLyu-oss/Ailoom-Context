...
def _render_context_storage_summary(payload: dict[str, Any]) -> str:
    lines = [
        ...
        f"Project root: {payload.get('project_root', '')}",
        f"Total generated bytes: {payload.get('total_bytes', 0)}",
        f"Total generated files: {payload.get('total_files', 0)}",
        "",
        "Generated artifacts:",
    ]
    for target in payload.get("targets") or []:
        lines.append(f"- {target.get('label', '')}: {target.get('bytes', 0)} bytes, {target.get('file_count', 0)} files, {target.get('path', '')}")
    lines.extend([
        "",
        "Recommended cleanup:",
...
def _build_context_storage_payload(args: argparse.Namespace) -> tuple[dict[str, Any], int]:
    ...
    status = "watch" if total_bytes >= 1024 * 1024 * 1024 else "ok"
    payload = {
        "status": "ok",
        ...
        "total_files": total_files,
        "targets": targets,
        "recommended_clean_command_text": _format_cli_command(["clean", "--dry-run", "--all", "--input-dir", str(project_root)]),
    }
