...
This writes JSON outputs for `version`, `handoff`, `savings`, `next`, `safety`, and `clean --dry-run`, plus `DEMO_SCREENSHOT_NOTES.md` for screenshots, `RECORDING_RUNBOOK.md` for a shot-by-shot recording checklist, and `SOCIAL_CAPTION.md` for copy/paste posting.
